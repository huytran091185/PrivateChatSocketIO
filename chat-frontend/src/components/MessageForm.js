        ...
        <>
            <div className="messages-output">
                ...
                {user && privateMemberMsg?._id && (
                    <>
                        <div className="alert alert-info conversation-info" style={{padding: '5px'}}>
                            {/* <div>
                ...
                {user &&
                    messages.map(({ _id: date, messagesByDate }, idx) => (
                        <div key={idx}>
                            <p className="alert alert-info text-center message-date-indicator" style={{'font-size':'0.8em'}}>{date}</p>
                            {messagesByDate?.map(({ content, time, from: sender }, msgIdx) => (
                                <div className={sender?.email == user?.email ? "message" : "incoming-message"} key={msgIdx}>
                                    <div className="message-inner">
                                        ...
                                        <p className="message-content" style={{'margin-bottom': '0px'}} >{content}</p>
                                        <p className="message-timestamp-right" style={{'margin-bottom': '0px'}} >{time}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
